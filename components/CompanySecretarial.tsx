...
const CompanySecretarial: React.FC = () => {
  ...
  return (
    <div className="bg-white min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="max-w-3xl text-center sm:text-left">
            <h1 className="text-4xl sm:text-5xl lg:text-7xl font-black mb-6 sm:mb-8 leading-tight tracking-tighter uppercase animate-fadeIn">
              ...
              <span className="text-gold">Company Secretarial</span> <br className="hidden sm:block"/>
              & Compliance
            </h1>
            ...
            <div className="flex justify-center sm:justify-start gap-6 animate-fadeIn" style={{ animationDelay: '0.4s' }}>
              <a href="#secretarial-contact" className="px-8 sm:px-10 py-4 sm:py-5 bg-gold text-navy-dark font-black rounded-lg hover:bg-white transition shadow-xl uppercase tracking-widest text-xs sm:text-sm w-full sm:w-auto text-center">
                Inquire Now
              </a>
            </div>
          </div>
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
            <div className="lg:w-1/2 relative animate-fadeIn">
              <div className="relative z-10">
                <div className="absolute -top-4 -left-4 sm:-top-6 sm:-left-6 w-24 sm:w-32 h-24 sm:h-32 bg-gold/10 rounded-2xl -z-10"></div>
                <img 
                  src="https://images.unsplash.com/photo-1507679799987-c73779587ccf?auto=format&fit=crop&q=80&w=2071" 
                  className="rounded-[30px] sm:rounded-[40px] shadow-2xl border-b-4 sm:border-b-8 border-gold w-full object-cover aspect-[4/3]" 
            ...
            <div className="lg:w-1/2 text-center lg:text-left">
              <h2 className="text-gold font-black text-[10px] sm:text-xs uppercase tracking-[0.3em] sm:tracking-[0.4em] mb-4">Strategic Foundation</h2>
              <h3 className="text-3xl sm:text-4xl lg:text-5xl font-black text-royal-blue mb-6 sm:mb-8 uppercase tracking-tight leading-tight">
                Why Corporate <br className="hidden sm:block"/>Governance Matters.
              </h3>
      ...
      <section className="py-16 sm:py-32 bg-slate-50 border-y border-slate-100 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16 sm:mb-24">
            <h2 className="text-gold font-black text-[10px] sm:text-xs uppercase tracking-[0.3em] sm:tracking-[0.5em] mb-4">Strategic Domains</h2>
            <h3 className="text-3xl sm:text-4xl lg:text-5xl font-black text-royal-blue uppercase tracking-tighter">Scope of Expertise</h3>
            <div className="w-16 sm:w-24 h-1.5 bg-gold mx-auto mt-6 rounded-full shadow-lg"></div>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {expertiseItems.map((s, i) => (
              <div key={i} className="group relative bg-white p-8 sm:p-10 lg:p-12 rounded-[24px] sm:rounded-[32px] shadow-sm border border-slate-100 hover:border-gold/50 hover:shadow-[0_20px_50px_rgba(0,51,102,0.1)] transition-all duration-500 cursor-default">
                <div className="absolute top-6 sm:top-8 right-8 sm:right-10 text-4xl sm:text-6xl font-black text-slate-200 select-none group-hover:text-gold/15 transition-colors">0{i+1}</div>
                <div className="w-12 h-12 sm:w-16 h-16 rounded-xl sm:rounded-2xl bg-royal-blue/5 flex items-center justify-center text-royal-blue mb-6 sm:mb-10 group-hover:bg-gold group-hover:text-white group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 shadow-sm relative z-10">
                  {s.icon}
                </div>
                <div className="relative z-10">
                  <h4 className="text-lg sm:text-xl font-black text-royal-blue mb-3 sm:mb-4 uppercase tracking-tight group-hover:text-gold transition-colors">{s.title}</h4>
                  <p className="text-slate-600 text-xs sm:text-sm font-medium leading-relaxed group-hover:text-slate-700 transition-colors">
                    {s.desc}
                  </p>
                </div>
                <div className="absolute bottom-0 left-8 sm:left-10 right-8 sm:right-10 h-1 bg-transparent group-hover:bg-gold transition-all duration-500 rounded-t-full"></div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 sm:py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl sm:text-3xl font-black text-royal-blue mb-12 sm:mb-16 text-center uppercase tracking-widest">Our Execution Roadmap</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div key={i} className="group relative p-[2px] rounded-[34px] overflow-hidden transition-all duration-500 hover:-translate-y-2">
                ...
                <div className="liquid-box relative h-full p-8 sm:p-10 bg-[#081b2a] text-white rounded-[32px] border border-white/5 shadow-2xl transition-all duration-500 overflow-hidden z-10">
                  <div className="liquid-wave"></div>
                  {/* Decorative Number (Background Overlay) */}
                  <div className="absolute -top-4 -right-2 text-7xl sm:text-9xl font-black text-white/[0.1] select-none group-hover:text-gold/10 transition-colors duration-500 pointer-events-none">
                    {p.step}
                  </div>
                  
                  <div className="w-12 h-1 bg-gold mb-6 sm:mb-8 rounded-full group-hover:w-20 transition-all duration-500"></div>
                  
                  <div className="relative z-10">
                    <h4 className="text-lg sm:text-xl font-black mb-3 sm:mb-4 uppercase tracking-tighter text-gold">{p.title}</h4>
                    <p className="text-blue-100/70 text-xs sm:text-sm font-medium leading-relaxed">{p.desc}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="py-16 sm:py-24 bg-slate-50 relative overflow-hidden">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full opacity-[0.01] pointer-events-none select-none flex items-center justify-center">
            <span className="text-[150px] sm:text-[400px] font-black uppercase text-navy-dark border-4 sm:border-8 border-navy-dark p-10 sm:p-20 rounded-full rotate-12">SSM</span>
        </div>
        
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-12 sm:mb-16">
            <h2 className="text-gold font-black text-[10px] sm:text-xs uppercase tracking-[0.3em] sm:tracking-[0.4em] mb-4">Common Enquiries</h2>
            <h3 className="text-3xl sm:text-4xl font-black text-royal-blue uppercase tracking-tight">Governance & Registration FAQ</h3>
          ...
          <div className="space-y-4">
            {faqs.map((faq, index) => (
              <div 
                key={index} 
                className={`border-2 rounded-xl sm:rounded-2xl transition-all duration-300 ${activeFaq === index ? 'border-gold bg-white shadow-xl' : 'border-slate-200 bg-white/50 hover:bg-white hover:border-slate-300'}`}
              >
                <button 
                  onClick={() => toggleFaq(index)}
                  className="w-full text-left px-6 sm:px-8 py-5 sm:py-6 flex items-center justify-between focus:outline-none"
                >
                  <div className="flex flex-col gap-1 pr-4">
                    <span className="text-[9px] sm:text-[10px] font-black text-gold uppercase tracking-widest">{faq.category}</span>
                    <span className="font-black text-royal-blue uppercase text-xs sm:text-sm tracking-tight leading-snug">{faq.question}</span>
                  </div>
                  <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${activeFaq === index ? 'border-gold bg-gold text-white rotate-180' : 'border-slate-200 text-slate-400'}`}>
          ...
          <div className="mt-12 sm:mt-16 text-center">
            <p className="text-slate-500 text-xs sm:text-sm font-bold uppercase tracking-widest mb-6">Have more specific questions?</p>
            <a href="https://wa.me/601124244993" className="inline-flex items-center gap-3 px-6 sm:px-8 py-4 bg-[#25D366] text-white font-black rounded-xl hover:scale-105 transition-all shadow-lg uppercase tracking-widest text-[10px] sm:text-xs w-full sm:w-auto justify-center">
      ...
      {/* 6. Lead Gen Footer */}
      <section className="py-16 sm:py-32 bg-white relative" id="secretarial-contact">
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          <div className="flex flex-col lg:flex-row bg-[#081b2a] rounded-[30px] sm:rounded-[50px] overflow-hidden shadow-[0_40px_100px_rgba(0,0,0,0.4)] border border-white/5 group">
            
            <div className="lg:w-2/5 p-8 sm:p-12 lg:p-20 bg-gradient-to-br from-navy-dark to-[#081b2a] border-r border-white/5 relative flex flex-col justify-center text-center lg:text-left">
              <div className="absolute top-10 left-10 w-24 h-24 bg-gold/5 rounded-full blur-3xl"></div>
              <div className="relative z-10">
                <div className="w-16 h-16 bg-gold/10 rounded-2xl flex items-center justify-center text-gold mb-8 border border-gold/20 shadow-lg shadow-gold/5 group-hover:scale-110 transition-transform duration-500 mx-auto lg:mx-0">
                  <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
                </div>
                <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black text-white mb-6 uppercase tracking-tight leading-tight">
                  The <span className="text-gold">BizFlow</span> <br className="hidden sm:block"/>Guarantee.
                </h2>
                <p className="text-blue-100/60 text-base sm:text-lg font-medium mb-10 sm:mb-12 leading-relaxed">
                  We handle the complex statutory hurdles of Malaysian compliance so you don't have to. Direct access to experts, not bots.
                </p>
                <div className="space-y-4 sm:space-y-6 text-left max-w-xs mx-auto lg:mx-0">
                  {["100% Data Confidentiality", "Licensed Secretarial Unit", "2-Hour Response Time", "No Hidden Retainer Fees"].map((item, i) => (
                    <div key={i} className="flex items-center gap-4">
                      ...
                      <span className="text-[10px] sm:text-xs font-black text-white uppercase tracking-widest opacity-80">{item}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="lg:w-3/5 p-8 sm:p-12 lg:p-20 relative">
              <div className="absolute bottom-0 right-0 w-64 h-64 bg-gold/5 rounded-full blur-3xl pointer-events-none"></div>
              <div className="max-w-xl mx-auto relative z-10">
                <h3 className="text-white font-black text-lg sm:text-xl uppercase tracking-widest mb-8 sm:mb-10 flex items-center gap-4 justify-center lg:justify-start">
                  <span className="w-6 sm:w-10 h-1 bg-gold"></span>
                  Start Your Inquiry
                </h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
                    <div className="relative group">
                      ...
                      <input 
                        ...
                        className="w-full px-0 py-3 bg-transparent border-b-2 border-white/10 focus:border-gold outline-none font-bold text-white transition-all placeholder:text-white/20 text-base sm:text-lg" 
                        required
                      />
                    </div>
                    <div className="relative group">
                      ...
                      <input 
                        ...
                        className={`w-full px-0 py-3 bg-transparent border-b-2 ${errors.email ? 'border-red-500' : 'border-white/10'} focus:border-gold outline-none font-bold text-white transition-all placeholder:text-white/20 text-base sm:text-lg`}
                        required
                      />
                      {errors.email && <p className="text-red-500 text-[9px] sm:text-[10px] font-black mt-2 uppercase tracking-widest animate-pulse">{errors.email}</p>}
                    </div>
                  </div>
                  <div className="relative group">
                    ...
                    <input 
                      ...
                      className={`w-full px-0 py-3 bg-transparent border-b-2 ${errors.phone ? 'border-red-500' : 'border-white/10'} focus:border-gold outline-none font-bold text-white transition-all placeholder:text-white/20 text-base sm:text-lg`}
                      required
                    />
                    {errors.phone && <p className="text-red-500 text-[9px] sm:text-[10px] font-black mt-2 uppercase tracking-widest animate-pulse">{errors.phone}</p>}
                  </div>
                  <div className="relative group">
                    ...
                    <textarea 
                      ...
                      className="w-full px-0 py-3 bg-transparent border-b-2 border-white/10 focus:border-gold outline-none font-bold text-white transition-all placeholder:text-white/20 text-base sm:text-lg resize-none h-24"
                      required
                    ></textarea>
                  </div>
                  <div className="pt-6 sm:pt-8">
                    <button className="w-full py-5 sm:py-6 bg-gold text-navy-dark font-black rounded-2xl hover:bg-white hover:scale-[1.02] transition-all shadow-2xl shadow-gold/20 uppercase tracking-[0.2em] sm:tracking-[0.3em] text-[10px] sm:text-xs flex items-center justify-center gap-4 group/btn">
                    ...
                    <div className="flex items-center justify-center gap-4 mt-8 opacity-40">
                      <span className="h-px w-8 sm:w-10 bg-white/50"></span>
                      <span className="text-[9px] sm:text-[10px] font-black text-white uppercase tracking-[0.2em]">Response within 60-120 mins</span>
                      <span className="h-px w-8 sm:w-10 bg-white/50"></span>
                    </div>
                  </div>
                </form>
